...
async def main_async(cfg: AppConfig):
    ...
    # Thought/probe should stay present-facing while life is active.  After the
    # idle threshold, quiet memory wandering may begin, seeded by recent KV.
    orch.kv_store.setdefault("probe:idle_wander_enabled", True)
    orch.kv_store.setdefault("probe:settling_threshold_s", 20.0)
    orch.kv_store.setdefault("probe:idle_wander_threshold_s", 90.0)
    orch.kv_store.setdefault("probe:current_window_s", 600.0)
    orch.kv_store.setdefault("probe:idle_candidate_threshold", 0.18)
    orch.kv_store.setdefault("probe:speak_threshold", 0.86)
    orch.kv_store.setdefault("probe:memory_drift_radius", 2)
    orch.kv_store.setdefault("probe:max_idle_candidates", 24)
    orch.kv_store.setdefault("probe:block_during_slearn_enabled", True)
    orch.kv_store.setdefault("probe:block_during_read_enabled", True)
    orch.kv_store.setdefault("probe:scene_delta_activity_threshold", 0.18)
